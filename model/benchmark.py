...
def get_emb(df, protein_col="Target Sequence", model="prost_t5"):
...
def load_dataset(dataset_name):
    ...
    train = pd.read_csv(base_path + "train.csv").dropna(subset=["SMILES", "Target Sequence", "Label"])
    test = pd.read_csv(base_path + "test.csv").dropna(subset=["SMILES", "Target Sequence", "Label"])
    valid = pd.read_csv(base_path + "val.csv").dropna(subset=["SMILES", "Target Sequence", "Label"])
...
def main():
    datasets = [
        "BindingDB",
        "BIOSNAP/full_data",
        "BIOSNAP/missing_data/70",
        "BIOSNAP/missing_data/80",
        "BIOSNAP/missing_data/90",
        "BIOSNAP/missing_data/95",
        "BIOSNAP/unseen_drug",
        "BIOSNAP/unseen_protein",
        "DAVIS",
    ]
    ...
    # AA_EMB += "_".join(datasets).replace("/", "_")
    ...
    for dt_str in ["14062024_0910", "02082024_0755"]:
        ...
        pd.DataFrame(flat_results).to_csv(f"results/all_results_detailed_{dt_str + '_' + AA_EMB + '_'.join(datasets).replace("/", "_")}.csv")
        pd.DataFrame(all_params).to_csv(f"results/all_params_{dt_str + '_' + AA_EMB + '_'.join(datasets).replace("/", "_")}.csv")
